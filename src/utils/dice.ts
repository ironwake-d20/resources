...
export function roll(
  ...
  if (keepCount === undefined || keepCount >= count) {
    ...
    const sorted = [...indexed].sort((a, b) => (keepBest ? b.value - a.value : a.value - b.value))
    const keptIndices = new Set(sorted.slice(0, keepCount).map((d) => d.index))
    dice = values.map((value, index) => ({ value, kept: keptIndices.has(index) }))
  }

  const keptSum = dice.filter((d) => d.kept).reduce((acc, d) => acc + d.value, 0)
