...
export default function CharacterSheet() {
  ...
  const [attributeMethod, setAttributeMethod] = useState<AttributeMethod>(saved.attributeMethod)
  ...
  const [conditions, setConditions] = useState<Attributes>(saved.conditions ?? DEFAULTS.conditions)
  ...
  const effectiveMaxLevel =
    kaiLevel !== ''
      ? (CAMPAIGN_KAI_LEVELS[campaign].find((k) => k.level === kaiLevel)?.maxLevel ?? MAX_LEVEL)
      : MAX_LEVEL
  ...
  function handleKaiLevelChange(raw: string) {
    ...
    const newMax =
      newKaiLevel !== ''
        ? (CAMPAIGN_KAI_LEVELS[campaign].find((k) => k.level === newKaiLevel)?.maxLevel ??
          MAX_LEVEL)
        : MAX_LEVEL
    if (level > newMax) {
      setLevel(newMax)
      setXp(XP_THRESHOLDS[newMax - 1])
    }
  }
  ...
  function handleKaiRoll() {
    ...
    setKaiPosition(band)
    if (kaiLevel !== '') setKaiValue(String((kaiLevel as number) * 1000 + band))
  }
  ...
  function handleLevelChange(raw: string) {
    const newLevel = Math.min(effectiveMaxLevel, Math.max(1, parseInt(raw) || 1))
    setLevel(newLevel)
    setXp(XP_THRESHOLDS[newLevel - 1])
  }
  ...
  function handleAttributeSwap(key1: keyof Attributes, key2: keyof Attributes) {
    setAttributes((prev) => ({ ...prev, [key1]: prev[key2], [key2]: prev[key1] }))
  }
    ...
    <div>
      <div className="flex items-end justify-between mb-8">
        <div>
          ...
          <p className="text-ctp-subtext1">Create and manage character sheets for your campaign.</p>
        </div>
      </div>
        ...
        <AttributesSection
          ...
          onMethodChange={(m) => {
            ...
            m === 'pointbuy'
              ...
              : setAttributes({ str: 1, dex: 1, con: 1, int: 1, wis: 1, cha: 1 })
          }}
